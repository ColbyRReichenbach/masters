...
export function PlayerSearchSelect({ players, value, onChange, compact = false }: PlayerSearchSelectProps) {
  return (
    <label className={`flex flex-col gap-2 ${compact ? "min-w-[190px]" : "min-w-[220px]"}`}>
      ...
      <select
        ...
        className={`${compact ? "h-10" : "h-11"} rounded-xl border border-masters-green/10 bg-white px-4 text-xs font-bold uppercase tracking-widest text-masters-green outline-none transition focus:border-masters-green/40`}
      >
        {players.map((player) => (
